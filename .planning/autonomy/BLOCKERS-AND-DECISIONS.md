...
**Closed out 2026-09-08.** Anthony registered the webhook endpoint (`we_1UCXWFJijfcmbDDBlfgn2lQQ`, live mode, `checkout.session.completed`) and added `STRIPE_WEBHOOK_SECRET`. Both confirmed independently: the endpoint shows up in Stripe's event-destination list, and probing the live function with a bad signature returned `401 invalid signature` (proof the secret exists) rather than the "not set" error. A full live-purchase round trip was NOT run -- Anthony chose to ship on the strength of that piece-by-piece verification rather than execute a real live-mode charge. Genuine residual risk, not hidden: see PRD-003's status section for what to check if the first real customer's purchase doesn't flow through.
