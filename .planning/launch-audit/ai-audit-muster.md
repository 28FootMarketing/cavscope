...
**Score 53 / 100, grade F, level 2 AI-assisted.** Models answer or draft inside the product; a person does the doing.
...
Generated 2026-09-07 by @bigsteele/ai-audit on 66 files. Every finding cites files; no source or values were read out. Env files were never opened.
...
- OpenRouter (1) — `supabase/functions/muster-agent/index.ts`
- Models named in code: claude-sonnet-5 (1) — `supabase/functions/muster-agent/index.ts`
- Prompts (system prompts, prompt files) (1) — `supabase/functions/muster-agent/index.ts`
- Structured output (schemas the model must fill) (1) — `supabase/functions/muster-agent/index.ts`

| Criterion | Earned |
|---|---|
| A model provider integrated | 6 / 8 |
| Prompts written down as code | 2 / 4 |
| Structured output the software acts on | 4 / 4 |
...
- Guardrails (rate limits 0, authenticated machine surfaces 5, moderation / redaction / cost caps 1) (6) — `supabase/functions/muster-agent/index.ts`, `supabase/functions/muster-alert-dispatch/index.ts`, `supabase/functions/muster-ghl-webhook/index.ts`, `supabase/functions/muster-scan/index.ts`, `supabase/functions/muster-watchdog/index.ts`, +1 more
