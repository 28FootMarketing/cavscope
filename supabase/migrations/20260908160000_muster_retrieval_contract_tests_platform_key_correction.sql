-- Corrects one test in muster.test_retrieval_contract().
--
-- The first version asserted that a p_ctx without organization_id must be
-- denied, and it failed. Reading muster_engine_resolve_api_key and
-- muster_engine_agent_call shows the test was wrong, not the code:
--
--   resolve_api_key returns 'organization_id', k.organization_id, and that
--   column is nullable. agent_call names the case outright --
--   'organization_id is required for platform keys' -- and applies the same
--   `if v_key_org is not null and v_org <> v_key_org` guard to every
--   website-scoped tool. A null org in the context IS a platform key, and a
--   platform key reading across organizations is the intended affordance.
--
-- So the invariant is not "deny a null org". It is: a null-org context reads
-- unscoped, therefore the ONLY thing gating that affordance is who can execute
-- the function at all. That is what made the anon grant a cross-tenant exposure
-- rather than a mere misconfiguration, and revoking the grant (see
-- 20260908130000) was the right fix. Hardening the ctx check would have broken
-- platform keys instead.
--
-- The replacement test ties the affordance to its actual control: it passes only
-- if a null-org context really is unscoped AND the function is unreachable by
-- anon and authenticated. Re-granting anon fails this test with an explanation,
-- not just a bare grant assertion.
...
begin
  ...
  -- A null-org context is a platform key and reads unscoped by design, matching
  -- muster_engine_agent_call. The control is therefore the grant, not the ctx.
  -- Both halves must hold together for this to pass.
  begin
    begin
      perform public.muster_engine_search_findings(
