-- ai_narrative: enforce the audience enum in SQL, not only in the tool's
-- advertised inputSchema. Before this, any string passed as audience was
-- interpolated straight into the model prompt ("Audience: <value>"), which
-- let a caller inject instructions into their own narrative request. The
-- blast radius was self-contained (an agent could only poison the output it
-- got back), but there is no reason to leave the door open.
